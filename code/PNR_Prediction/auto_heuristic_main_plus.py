...
from utils import read_graph, normalize_matrix, normalize_matrix_full, get_trainset_path, get_testset_path, get_test_matrix_binary, \
    save_DNN_hybrid_scores, save_plus_raster_scores, save_plus_hybrid_scores
...
def auto_plus(prex=None,
    ...
    if os.path.exists(path_scores_method1) and os.path.exists(path_scores_method2):
        ...
        #                                          scores_matrix_one_norm=scores_matrix_one_norm,
        #                                          scores_matrix_two_norm=scores_matrix_two_norm)
        scores_matrix_plus = csr_matrix(0.5 * csr_matrix(scores_matrix_one).A + 0.5 * csr_matrix(scores_matrix_two).A)


        # 以下相加的这两种结果是不一样的！！！因为一般来说，分数大的归一化后他的影响力就小了！！
        # scores_matrix_plus = 0.5*csr_matrix(scores_matrix_one) + 0.5*csr_matrix(scores_matrix_two)
        # scores_matrix_plus = 0.5 * csr_matrix(scores_matrix_one_norm) + 0.5 * csr_matrix(scores_matrix_two_norm)

        # scores_matrix_plus = csr_matrix(csr_matrix(scores_matrix_one).A * csr_matrix(scores_matrix_two).A)

        # scores_matrix_plus = csr_matrix(np.plus(scores_matrix_one_norm.A,
        ...
        save_plus_hybrid_scores(scores_matrix_plus=scores_matrix_plus,
        ...
        save_plus_raster_scores(rastser_grids=plus_raster_grids,
                               method1=emb_method_name1, method2=emb_method_name2,
                               graph_results_dir=graph_results_dir,
                               dataset_name=graph_name,
                               plus_binNum=plus_binNum)
        ...
        nonexist_scores_plus_list = (np.array(scores_matrix_plus_norm[nonexist_binary > 0], dtype=float))[0]
        L_full = int(np.sum(test_binary))
        L_array = np.array([int(L_full / 20), int(L_full / 10),
                            int(L_full / 5), int(L_full / 2),
                            L_full])
        AP_plus, AUC_plus, Precision_plus, Recall_plus, F1score_plus = \
            evaluators(train_binary=train_binary,
                       test_binary=test_binary,
                       scores_list=nonexist_scores_plus_list,
                       L_array=L_array)
        ...
        plus_write_to_excel(dataset_name=graph_name, method1=emb_method_name1, method2=emb_method_name2,
                           precision_plus=Precision_plus,
                           recall_plus=Recall_plus,
                           F1score_plus=F1score_plus,
                           AP_plus=AP_plus)
...
if __name__ == '__main__':
    ...
    plus_binNum = 50  # DNN rasterization grids # 改这里
...
for method_i in range(len(methods_heuristic)):
  for method_j in range(method_i+1, len(methods_heuristic)):
    ...
    emb_method_name2 = methods_heuristic[method_j] # 改这里
    ############################### 第一个baseline##########################
    ...
    for i in range(len(graph_group_micro)):
        ...
        auto_plus(prex=prex, graph_name=graph_name,
