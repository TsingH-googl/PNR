...
from utils import read_graph, normalize_matrix, normalize_matrix_full, get_trainset_path, get_testset_path, get_test_matrix_binary, \
    save_DNN_hybrid_scores, save_DNN_raster_scores
...
from fileIO_utils import DNN_write_to_excel, is_excel_file_exist
...
def auto_DNN(prex=None,
             graph_name=None, emb_method_name1=None, emb_method_name2=None,
             model_name=None, DNN_binNum=None):
    print('----------------------------------------------------------')
    ...
    if os.path.exists(path_scores_method1) and os.path.exists(path_scores_method2):
        ...
        scores_matrix_one_norm = normalize_matrix(csr_matrix1=csr_matrix(scores_matrix_one))
        ...
        scores_matrix_DNN_norm = normalize_matrix(csr_matrix1=scores_matrix_DNN)



        # 计算DNN的rasterization grids
        DNN_raster_grids = rasterization_grids(binNum=DNN_binNum,
                                               train_binary=train_binary,
                                               scores_matrix_DNN=scores_matrix_DNN_norm,
                                               scores_matrix_one_norm=scores_matrix_one_norm,
                                               scores_matrix_two_norm=scores_matrix_two_norm)
        # DNN_raster_grids = np.log10(DNN_raster_grids) # 出现-inf而报错
        DNN_raster_grids = normalize_matrix_full(csr_matrix1=csr_matrix(DNN_raster_grids))
        ...
        nonexist_scores_DNN_list = (np.array(scores_matrix_DNN[nonexist_binary > 0], dtype=float))[0]
        ...
        DNN_write_to_excel(DL_name=model_name, dataset_name=graph_name, method1=emb_method_name1, method2=emb_method_name2,
...
if __name__ == '__main__':
    ...
    DNN_binNum = 50  # DNN rasterization grids # 改这里
  ...
  for method_j in range(method_i+1, len(methods_heuristic)):
    ...
    if prex=='preprocessing_code//':
        ...
        graph_group_min = [
            ...
            # 'wiki',
            'facebook_combined'] # 5个 # 'petster-hamster','yeastprotein','ca-grqc', 'cora', 'citeseer','usairport', 'openflights'
        ...
        graph_group_micro = [] # 4个'brazil-airports','NS','PB',
        graph_group_min = [#'usa-airports',
                            'yeast']# 3个 # ,'ppi2','europe-airports','citeseer2_mat','citeseer2',
    ...
    for i in range(len(graph_group_micro)):
        ...
        auto_DNN(prex=prex, graph_name=graph_name,
                 emb_method_name1=emb_method_name1,
                 emb_method_name2=emb_method_name2,
                 model_name=model_name, DNN_binNum=DNN_binNum)
    pass
