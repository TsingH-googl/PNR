...
xls_file_path  = 'D:\链路预测相关课题\matlab评估代码\实验结果//MAP.xls'
...
hspace=0.2
wspace=0.45
my_x_ticks = np.arange(min(x_data), max(x_data) + 0.01, 5)
...
for i in range(7):
    ...
    plt.plot(x_data, ml100k_rnr_data[i], marker='o', markersize = markersize,label=rnr_baselines[i], color=dataset_colors[0])
    plt.xticks(my_x_ticks, fontsize=ticks_fontsize)
    plt.yticks(fontsize=ticks_fontsize)
    plt.legend(fontsize=legend_fontsize)
    if i==0:
        plt.ylabel("MAP@N", fontsize=x_y_label_fontsize)
    if i==3:
    ...
    # plt.xlim(x_data[0], x_data[3]+0.1)
...
for i in range(7):
    ...
    plt.plot(x_data, wikibooks_rnr_data[i], marker='o', markersize = markersize,label=rnr_baselines[i], color=dataset_colors[3])
    plt.xticks(my_x_ticks, fontsize=ticks_fontsize)
    plt.yticks(fontsize=ticks_fontsize)
    plt.legend(fontsize=legend_fontsize)
    if i==0:
...
plt.savefig('./figures//' + 'MAP_all.eps', format='eps',
            bbox_inches='tight',
