...
# Wikibooks：18/15
row_start = 1 # 改这里
col_start = 2 + topL # 改这里
plt.title("ML100K") # 改这里
y_data = []
y_data2 = []
for i in range(7):
    y_data.append(float(table.cell(row_start + 2*i, col_start).value))
    y_data2.append(float(table.cell(row_start + 1 + 2*i, col_start).value))
    pass
...
ax.xaxis.get_offset_text().set_fontsize(ticks_fontsize)
...
plt.yticks(color='black')
# plt.xlim(0.004, 0.01)
# my_x_ticks = np.arange(0.004, 0.01, 0.001)
# plt.xticks(my_x_ticks)
ax = plt.gca()
...
plt.savefig('./figures//' + xlabel +'.eps', format='eps', dpi=600,
            bbox_inches='tight',
