...
save_xls_file='D:\hybridrec\临时文件夹//heuristic_AP.xls'
...
for datasets_index in range(1, len(datasets)+1):
    ...
    y_data1 = []
    y_data2 = []
    n = len(methods_heuristic)
    rotation = 90  # 改这里

    for i in range(len(methods_heuristic)):
        for j in range(i + 1, len(methods_heuristic)):
            val = methods_heuristic[i] + '+' + methods_heuristic[j]
...
plt.savefig('D:\hybridrec\临时文件夹//' +'heuristic_AP.eps', format='eps',
