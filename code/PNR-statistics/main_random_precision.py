...
for method_pair_index in range(len(method_pairs1)):
    ...
    x_data.append('MLP')
    x_data.append('PNR')
    y_data.append(method1_precision)
    y_data.append(method2_precision)
    y_data.append(plus_precision)
    y_data.append(multiply_precision)
    y_data.append(MLP_precision)
    y_data.append(PNR_precision)
...
plt.savefig('D:\hybridrec\临时文件夹//' +dataset+'_precision_demo.eps', format='eps',
            bbox_inches='tight') # pad_inches使得四周的空白都去掉了
plt.savefig('D:\hybridrec\临时文件夹//' +dataset+'_precision_demo.png', format='png',
            bbox_inches='tight') # pad_inches使得四周的空白都去掉了
plt.savefig('D:\hybridrec\临时文件夹//' +dataset+'_precision_demo.pdf', format='pdf',
            bbox_inches='tight') # pad_inches使得四周的空白都去掉了
plt.show()
