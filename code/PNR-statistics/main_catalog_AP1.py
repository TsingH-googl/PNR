...
save_xls_file='D:\hybridrec\临时文件夹//heuristic_embedding_AP.xls'
...
for datasets_i in range(len(datasets)):
    for method_i in range(len(methods_heuristic)):
        for method_j in range(len(methods_embedding)):
            ...
            n1 = len(methods_heuristic) + len(methods_embedding)
            n2 = len(methods_heuristic) * len(methods_embedding)
            ...
            table_write.write(start_row+len(methods_heuristic) + method_j,
                            0,
            ...
            # 写MLP的结果
            combine_index=int(method_i*len(methods_embedding) + method_j)

            table_write.write(start_row+n1+combine_index,
                                0,
                                'MLP-('+methods_heuristic[method_i] + '+' + methods_embedding[method_j] + ')',
                                style=xlwt.XFStyle())
            ...
            # # 写PNR的结果
            combine_index=int(method_i*len(methods_embedding) + method_j)
            table_write.write(start_row+n1+n2+combine_index,
                            0,
                            'PNR-('+methods_heuristic[method_i] + '+' + methods_embedding[method_j] + ')',
                             style=xlwt.XFStyle())
...
table = data.sheets()[0]
fig = plt.figure(figsize=(10, 18)) # figsize=(17, 3)或1

for datasets_index in range(1, len(datasets)+1):
    ax = fig.add_subplot(4, 1, datasets_index)
    ...
    for i in range(len(methods_heuristic)):
        for j in range(len(methods_embedding)):
            val = methods_heuristic[i] + '+' + methods_embedding[j]
            val = val.replace('degreeproduct', 'DP')
            x_data.append(val)
        pass
    pass
    ...
    # 读取MLP数据
    for y_data_index in range(n2):
        val = table.cell(start_row + n1 + y_data_index,
                         datasets_index).value
        y_data1.append(val)
    pass
    ...
    # 读取PNR数据
    for y_data_index in range(n2):
        val = table.cell(start_row + n1 +n2 + y_data_index,
                         datasets_index).value
        y_data2.append(val)
    pass

    plt.bar(np.arange(len(x_data)), y_data1,
            label='MLP', width=bar_width,
            color='steelblue', alpha=alpha, hatch=hatch1)
    plt.bar(np.arange(len(x_data)) + bar_width, y_data2,
            label='PNR', width=bar_width,
    ...
    offset = 0.5
    # for i in range(len(methods_heuristic)):
    #     plt.annotate(methods_heuristic[i].replace('degreeproduct', 'DP'),
    #                  xy=(int(n2), baseline_data[i]),
    #                  xytext=(int(n2) + offset, baseline_data[i])
    ...
    #                  xy=(int(n2), baseline_data[len(methods_heuristic)+i]),
    #                  xytext=(int(n2) + offset, baseline_data[len(methods_heuristic)+i])
    #                  )  # , arrowprops=dict(facecolor='red', shrink=0.1, width=2)
    #     plt.hlines(baseline_data[len(methods_heuristic)+i],
    #                0 - offset,
    #                int(n2) + offset,
    #                linestyles='--', colors='black')
    # pass
...
plt.savefig('D:\hybridrec\临时文件夹//' +'catalog_AP.eps', format='eps',
